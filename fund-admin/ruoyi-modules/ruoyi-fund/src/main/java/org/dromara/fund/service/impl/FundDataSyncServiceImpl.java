...
public class FundDataSyncServiceImpl implements IFundDataSyncService {
    ...
    private PersistResult persistCatalog(
        ...
        return transactionTemplate.execute(status -> {
            ...
            return new PersistResult(resolveState(valid.size(), issues.size(), 0), changed);
        });
    }
    ...
    private PersistResult persistFundBundle(
        ...
        return transactionTemplate.execute(status -> {
            ...
            return new PersistResult(resolveState(fundInfos.size() + navItems.size() + holdingItems.size(), issues.size(), 0), changed);
        });
    }
    ...
    private record PersistResult(String state, int changedCount) {
    }
}
